        ...
        public class CalendarLayoutEngineTests
        {
            ...
            public void CalculateDayRects_CalculatesCorrectPositions()
            {
                ...
                for (int i = 0; i < TotalCells; i++)
                {
                    ...
                    int expectedY = row * CellSize + HeaderHeight + CellPadding;
            ...
            public void GetDayFromLocation_ReturnsCorrectDay()
            {
                ...
                var day1 = engine.GetDayFromLocation(new Point(5, HeaderHeight + 5), CellSize, HeaderHeight, startOffset, daysInMonth);
                Assert.That(day1, Is.EqualTo(1));

                var day7 = engine.GetDayFromLocation(new Point(6 * CellSize + 1, HeaderHeight + 1), CellSize, HeaderHeight, startOffset, daysInMonth);
                Assert.That(day7, Is.EqualTo(7));

                var day8 = engine.GetDayFromLocation(new Point(1, HeaderHeight + CellSize + 1), CellSize, HeaderHeight, startOffset, daysInMonth);
            ...
            public void GetDayFromLocation_ReturnsNullWhenOutside()
            {
                ...
                var day = engine.GetDayFromLocation(new Point(-5, HeaderHeight + 5), CellSize, HeaderHeight, startOffset, daysInMonth);
                Assert.That(day, Is.Null);

                day = engine.GetDayFromLocation(new Point(10, HeaderHeight + 6 * CellSize + 1), CellSize, HeaderHeight, startOffset, daysInMonth);
                Assert.That(day, Is.Null);

                day = engine.GetDayFromLocation(new Point(7 * CellSize + 1, HeaderHeight + 1), CellSize, HeaderHeight, startOffset, daysInMonth);
                Assert.That(day, Is.Null);
            }
        }
    }